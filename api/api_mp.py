...
class ApiMp:
    ...
    def api_mp_article(self, title, content, channel_id):
        ...
        """
        log.info('-' * 200)
        log.info("正在调用自媒体发布文章接口")
        # 1.定义请求数据
        data = {"title": title, "content": content, "imgType": "auto",
                "channel_id": channel_id, "cover": {"type": -1, "images": []}}
        log.info("请求数据：{}".format(data))
        # 2.调用post方法
        interface = requests.post(url=self.url_article, json=data, headers=api.headers)
        log.info("请求方法：post 请求数据：{} 请求头：{}".format(data, api.headers))
        return interface
